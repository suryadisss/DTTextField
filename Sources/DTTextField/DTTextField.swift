...
open class DTTextField: UITextField {
    ...
    public var errorMessage:String = ""{
        didSet{ lblError.text = errorMessage
            lblError.accessibilityIdentifier = placeholderFinal
            lblError.accessibilityLabel = errorMessage
        }
    }
...
//            guard showErrorLabel != oldValue else { return }
    ...
    public var placeholderSpesific:String = ""{
        didSet{
            shouldIgnoreDidSet = true
            placeholder = placeholderSpesific
        }
    }
    ...
    @objc fileprivate func textFieldTextChanged(){
        //floatPlaceholderFont = UIFont(name:"Inter-Medium", size: 10.0) ?? UIFont.systemFont(ofSize: 10)
    }
